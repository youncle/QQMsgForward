...
| 文件/目录 | 代码体量 | 核心职责 |
| ---- | ---- | ---- |
| `tray.py` | ~828 行 | 项目主入口、全生命周期管理；多 LLBot 启停、托盘图标、启动动画、配置窗口、Flask 守护线程、配置持久化、快捷方式管理 |
| `forward.py` | ~279 行 | 消息转发核心引擎；WebHook 接收、消息去重、群限流、API 轮询、消息重发重试 |
| `filter.py` | ~250 行 | 消息过滤引擎；二维码拦截、联系方式正则检测、群白名单豁免 |
...
| `LLBot-CLI-Win-x64/` | - | LLOneBot v7.11.4 运行时（Node.js + 协议组件） |
...
| 2 | LLBot-CLI-Win-x64-3 | 3002 | 3082 |
...
## 十一、未完善模块 & 现存短板
### 11.1 空白目录（未落地）
- `tests/`：目录存在，无任何单元测试、功能测试用例
- `openspec/`：曾规划设计文档，现已废弃空置
- 附属组件：`pmhq.dll/pmhq.exe`（QQ 协议/签名组件）、LLOneBot 原生 WebUI，Python 层未做封装调用

### 11.2 潜在风险与优化点
1. **进程管理**：仅全局查杀进程，未托管 `subprocess` 句柄，关闭逻辑不够精细
2. **多实例启动**：每次复制 150MB+ 完整目录，实例较多时启动缓慢
3. **配置能力**：不支持**热重载**，修改规则必须重启全服务
4. **状态检测**：仅校验端口开放，不检测 QQ 实际登录状态，易出现静默发送失败
5. **Web 服务**：直接使用 Flask 内置开发服务器跑生产，无专业 WSGI 容器
6. **去重缓存**：纯内存缓存，进程重启丢失、无最大容量限制，存在内存溢出风险