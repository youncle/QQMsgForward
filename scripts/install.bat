...
:: 3. 清理旧日志
echo [3/4] 清理运行时日志文件 ...
if exist logs\forward.log del logs\forward.log
if exist config\.window_state.json del config\.window_state.json
if exist "runtime\LLBot-CLI-Win-x64\bin\llbot\data\logs" (
    del /q "runtime\LLBot-CLI-Win-x64\bin\llbot\data\logs\*.log" 2>nul
)
echo    日志已清理
